...
- The **Vault Context** block (plugin contract + MUST rules) is shared across every skill and agent, and the **Summary Page Spec** region is shared between `skills/wiki-ingest/SKILL.md` and `agents/ingest-worker.md`. Both are single-sourced: the canonical content lives in `shared/vault-context.md` and `shared/summary-page-spec.md`, and `scripts/sync-shared-blocks.sh` injects it between each carrier's `<!-- Vault Context ... -->` / `<!-- /Vault Context -->` (and `<!-- Summary Page Spec ... -->` / `<!-- /Summary Page Spec -->`) markers, re-indented to match the carrier. Never edit the copies in `skills/` or `agents/` — edit the `shared/*.md` file and run the script. Keep both marker lines intact in every carrier; CI fails if a marker is missing or the copies drift from `shared/*.md`.
