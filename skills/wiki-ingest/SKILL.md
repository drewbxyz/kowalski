...
2. Dispatch one `kowalski:ingest-worker` agent per source, in parallel. Each worker processes its source's single-source flow (section 3) **except** the shared files and any edit to an existing page — workers do not touch `${user_config.wiki_root}/index.md`, `${user_config.wiki_root}/log.md`, `${user_config.wiki_root}/hot.md`, `${user_config.sources_dir}/manifest.json`, **any folder `_index.md`** (two workers filing into the same folder would race on its `_index.md`), **or any existing wiki page** (two workers editing the same entity page would read-modify-write over each other). Workers only create new pages, re-checking just before each write that a sibling worker hasn't already created the file. Each worker returns: pages created, proposed edits to existing pages (path, what to change, why), proposed `_index.md` lines (folder → line), its proposed manifest entry (with hash), a key insight, any contradictions found, and any needs-filing items (content the vault has no precedent for — workers never create new folders).
3. Once all workers report back, the orchestrator does one consolidation pass: it applies every worker's proposed edits to existing pages — when two workers propose edits to the same page, merge them into a single coherent edit (resolve overlapping content rather than appending both); it detects duplicate creations — two workers reporting the same created page path means the file was written twice and only the last write survived, so reconcile the surviving content against both workers' reports and merge back anything the overwrite lost; it does one cross-reference pass across the newly created/updated pages (link related entities to each other, resolve any contradictions flagged by multiple workers); it applies every worker's proposed `_index.md` lines to the relevant folder catalogs; and it consolidates all needs-filing items into one question to the user — proposing 1–2 options per item, grounded in the vault's existing shape — then files them and registers any agreed new folders.
...
**Single-writer rule: only the orchestrator writes `${user_config.wiki_root}/index.md`, `${user_config.wiki_root}/log.md`, `${user_config.wiki_root}/hot.md`, `${user_config.sources_dir}/manifest.json`, and — in batch mode — all folder `_index.md` files AND all existing wiki pages. Workers never write these files or edit an existing page, even when running in parallel — workers only create new pages, and every change to an existing page goes through the orchestrator as a proposed edit. This is what prevents concurrent writers from corrupting the shared catalogs, log, cache, manifest, and entity pages.**
